...
public interface MdYoloResultRepository extends JpaRepository<MdYoloResult, Long> {
    ...
    @Query(value = "select distinct(img_path) from md_yolo_result where user_id = :username", nativeQuery = true)
    public List<String> findAllImgIDbyUserId(@Param(value="username") String username);
}